package ru.pgk63.core_ui.view.dialog.calendar.utils
...
internal val LocalDate.previousWeek: LocalDate
    ...
    get() {
        val dateNew = when {
            dayOfMonth == DialogConstants.FIRST_DAY_IN_MONTH && dayOfWeek != DayOfWeek.MONDAY -> {
                // Skips in current week to previous month's monday
                with(DayOfWeek.MONDAY)
            }
            dayOfMonth >= DialogConstants.DAYS_IN_WEEK || dayOfMonth == DialogConstants.FIRST_DAY_IN_MONTH
                ...
                withDayOfMonth(DialogConstants.FIRST_DAY_IN_MONTH)
            }
        }
        return dateNew
    }
...
internal val LocalDate.nextWeek: LocalDate
    ...
    get() {
        ...
        val daysUntilNextMonday = (DialogConstants.DAYS_IN_WEEK - dayOfWeek.value).plus(1).toLong()
        val dateNew = when {
            dayOfMonth == DialogConstants.FIRST_DAY_IN_MONTH -> {
                // Skip to next monday
                plusDays(daysUntilNextMonday)
            }
            daysInMonthLeft >= DialogConstants.DAYS_IN_WEEK -> {
                // Skip to next week
                plusWeeks(1)
            }
            else -> {
                // Last day in month or less than a week of days left in month, skip to begin next month
                plusMonths(1).withDayOfMonth(DialogConstants.FIRST_DAY_IN_MONTH)
            }
        }
        return dateNew
    }
...
internal val List<LocalDate?>.startValue: LocalDate?
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
    get() = this.getOrNull(DialogConstants.RANGE_START)
...
internal val List<LocalDate?>.endValue: LocalDate?
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
    get() = this.getOrNull(DialogConstants.RANGE_END)
...
internal fun calcCalendarData(config: CalendarConfig, cameraDate: LocalDate): CalendarData {
    ...
    val offsetStart = when (config.style) {
        CalendarStyle.MONTH -> cameraDate.withDayOfMonth(DialogConstants.FIRST_DAY_IN_MONTH).dayOfWeek.ordinal
        CalendarStyle.WEEK -> {
            val dayOfWeekInMonth = cameraDate.withDayOfMonth(DialogConstants.FIRST_DAY_IN_MONTH).dayOfWeek
            val diff = dayOfWeekInMonth.ordinal
            val value = if (weekCameraDate.dayOfMonth <= DialogConstants.DAYS_IN_WEEK && diff > 0) {
