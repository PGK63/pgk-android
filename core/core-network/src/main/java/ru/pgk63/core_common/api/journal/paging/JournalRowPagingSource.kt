...
): PagingSource<Int, ru.pgk63.core_model.journal.JournalRow>() {

    override fun getRefreshKey(state: PagingState<Int, ru.pgk63.core_model.journal.JournalRow>): Int? {
        val anchorPosition = state.anchorPosition ?: return null
        val page = state.closestPageToPosition(anchorPosition) ?: return null
        return page.prevKey?.plus(1) ?: page.nextKey?.minus(1)
    }

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ru.pgk63.core_model.journal.JournalRow> {
        return try {
